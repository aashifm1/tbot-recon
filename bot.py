...
def fetch_live_subs(subs_path: Path, folder_path: Path):
    ...
    for sub in subs:
        thread = threading.Thread(target=check_live, args=(sub, live_queue))
        thread.start()
    ...
    while not live_queue.empty():
        live_subs.append(live_queue.get())
...
async def handle_domain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        live_subs_path = fetch_live_subs(subs_path, folder_path)
